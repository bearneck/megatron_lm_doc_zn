...
@deprecated(
    version="1.0.0",           # 弃用开始时间
    removal_version="2.0.0",    # 将被移除的时间
    alternative="new_function", # 推荐的替代方案
    reason="Improved performance and cleaner API" # 原因：性能改进和更简洁的 API
)
def old_function(x):
    """此函数已弃用。"""
    pass
...
# ✅ 之后 (v1.1) - 可以自由更改
@internal_api
def _internal_compute(x, y, z):  # 添加了参数
    pass
```
**结果：** ✅ 检查通过（函数被豁免）
...
GitHub Actions 触发
    ↓
CI 运行 check_api_backwards_compatibility.py
    ↓
脚本通过 griffe 加载代码：
  • 基线：最新发布版本 (例如，core_r0.8.0)
  • 当前：PR 分支
    ↓
应用过滤：
  • 跳过 @internal_api、@experimental_api 和 @deprecated
  • 跳过私有函数 (_前缀)
  • 跳过测试/实验路径
    ↓
Griffe 比较签名：
...
```yaml
# 在 .github/workflows/check_api_backwards_compatibility_workflow.yml 中
