...
以下是一些可能有用的环境变量。
```bash
export PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True # 启用可扩展段以防止内存碎片
export NCCL_NVLS_ENABLE=0 # 禁用 NVLS 以防止内存开销
```
#### 依赖项
...
--offload-modules expert_fc1 moe_act # 选项：attn_norm, core_attn, attn_proj, mlp_norm, expert_fc1, moe_act
```
...
# 在 32 个 H100/A100 GPU 上运行 Mixtral 8x7B 模型
