...
MEGATRON_PATH=${MEGATRON_PATH:-"your_own_megatron_path"} # Path to the Megatron-LM repository
CONTAINER_IMAGE=${CONTAINER_IMAGE:-"your_own_container_image"} # Path to .sqsh file or docker image URL
...
# Submit SLURM job
# Note: Update the SBATCH parameters below according to your cluster configuration
set +e
sbatch <<EOF
#!/bin/bash
